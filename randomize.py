...
def get_data():
	num_items = 100
	data = np.zeros((len(train_folders)*num_items, img_height, img_width, 3))
	# labels = np.zeros((len(train_folders)*num_items, num_classes))
	count = 0

	for (i, file) in enumerate(train_folders):
		f = h5py.File(file, "r")
...
def randomize():
	...
	print('Saving randomized random_data%d' % num_items)
...
shuffled_data, shuffled_labels = randomize()
